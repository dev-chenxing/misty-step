...
local wallOfForceSpell = tes3.getObject(constants.SPELL_ID)
if not wallOfForceSpell then
    wallOfForceSpell = tes3.createObject({
        id = constants.SPELL_ID,
        objectType = tes3.objectType.spell
    })
    tes3.setSourceless(wallOfForceSpell)
    wallOfForceSpell.name = constants.MOD_NAME
    wallOfForceSpell.magickaCost = constants.MAGICKA_COST
...
local function addSpellToMerchant(e)
    ...
    ---@cast npc tes3npc
    if not npc.aiConfig.offersSpells then return end

    if config.spellMerchants[e.target.id] or
        config.spellMerchants[e.target.baseObject.id] then
        local wasAdded = tes3.addSpell({
            reference = e.target,
            spell = wallOfForceSpell,
            updateGUI = false
        })
        if wasAdded then
            log:debug("Added Wall of Force spell to %s", e.target.id)
        end
    end
    ...
    if config.scrollMerchants[e.target.id] or
        ...
        if npc.aiConfig.bartersBooks then
            ...
            if npc.inventory:contains(scroll) then
                log:debug("Added Wall of Force scroll to %s's inventory",
                          e.target.id)
            end
        end
    end
